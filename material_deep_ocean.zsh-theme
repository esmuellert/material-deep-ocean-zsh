...
PROMPT='$(_user_host)$(git_prompt_info)${_current_dir}%{$white%}%#%{$reset_color%} '
PROMPT2='%{$grey%}◀%{$reset_color%} '
...
function _user_host() {
  echo "%{$red%}%n%{$reset_color%}%{$red%}@%{$red%}%m%{$reset_color%} %{$white%}"
}
...
ZSH_THEME_GIT_PROMPT_DIRTY=" %{$red%}✗%{$reset_color%}%{$blue%}"
ZSH_THEME_GIT_PROMPT_CLEAN=" %{$green%}✔%{$reset_color%}%{$blue%}"
ZSH_THEME_GIT_PROMPT_BEHIND_REMOTE="%{$red%}⬇%{$reset_color%}%{$blue%}"
ZSH_THEME_GIT_PROMPT_AHEAD_REMOTE="%{$green%}⬆%{$reset_color%}%{$blue%}"
ZSH_THEME_GIT_PROMPT_DIVERGED_REMOTE="%{$yellow%}⬌%{$reset_color%}%{$blue%}"
